...
export class DataFieldsManagementComponent implements OnInit {
    ...
    this.itemsMenu = [
      {
        label: 'Opciones',
        items: [
        { label: 'Editar título del grupo', icon: 'pi pi-pencil', command: () => this.startGroupEdit(this.selectedGroupItem) },
        { label: 'Eliminar grupo', icon: 'pi pi-trash', command: () => this.removeGroup(this.selectedGroupItem) }
        ]
      }
    ];
  }
  ...
  loadEssentialData() {
    ...
    this.productService.getApiData(`complementaryData/findEssentialDataByInsuranceLineApplicationLevel/${parameters}`).subscribe({
      next: (res: any) => {
        console.log('res', res);
        if (res && res.body) {
   ...
   */
  loadRequiredData() {
    this.productService.getApiData(`displayEssential/findByProcess/` + this.productService.initialParameters?.get('insuranceLine')?.value + `/0/0/0/0`).subscribe({
      next: (res) => {
        console.log('res', res);
      },
      error: (error) => {
  ...
  /*openDialog(code: string): void {
    ...
    switch (code) {
      case 'typeCurrencyControls':
        columns = [
          { field: 'name', header: 'Nombre', displayValue: ['officialName'] },
          { field: 'description', header: 'Descripción', displayValue: ['isoCode'] }
        ];
        sendData = this.productService.initialParameters.get('typeCurrency')!.value
        break;
      case 'objectiveGroupControls':
        columns = [
          { field: 'name', header: 'Nombre', displayValue: ['name'] },
          { field: 'description', header: 'Descripción', displayValue: ['description'] }
        ];
        sendData = this.productService.initialParameters.get('objectiveGroup')!.value
        break;
      case 'salesChannelControls':
        columns = [
          { field: 'name', header: 'Nombre', displayValue: ['nmName'] },
          { field: 'description', header: 'Descripción', displayValue: ['dsDescription'] }
        ];
        sendData = this.productService.initialParameters.get('salesChannel')!.value;
        break;
      case 'billingPeriodControls':
        ...
        sendData = this.productService.initialParameters.get('billingPeriod')!.value
        break;
      default:
        break;
    };
    this.openDialog(code, sendData, columns);
  }*/
  ...
  openDialogAddData() {
    const columns = [
      { field: 'name', header: 'Nombre', displayValue: ['nmLabel'], dbColumnName:['nmLabel'] },
      { field: 'dataType.description', header: 'Descripción', displayValue:['dataType.description'], dbColumnName:['dsDescription'] },
 ...
 */
  ...
  addNewGroup() {
    ...
    this.startGroupEdit(this.complementaryDataControls.controls[this.complementaryDataControls.controls.length - 1]);
  }
  ...
  removeGroup(group: any) {
    ...
    let dialogRef = this.dialogService.open(ModalDeleteComponent, {
      data: {
        ...
        subMessage: 'Los datos asociados al grupo pasarán al grupo de datos básicos'
      },
      showHeader: false,
      width: '400px'
    });
    ...
    dialogRef.onClose.subscribe((res) => {
      if (res) {
        if (index >= 0) {
          ...
          if (this.groups){this.DeleteCascadeDateModify(1);}
